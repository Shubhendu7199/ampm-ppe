...
resource "azurerm_servicebus_namespace" "servicebus_namespace" {
  ...
  name                = "sbns-wpp-wt-ampm-${var.client_name}-${module.location-lookup.location-lookup["location_short"]}-${var.environment}-${each.key}"
...
resource "azurerm_servicebus_namespace_authorization_rule" "namespace_authorization_rule" {
  ...
  name         = "sbns-authrule-wpp-wt-ampm-${var.client_name}-${module.location-lookup.location-lookup["location_short"]}-${var.environment}-${each.value.auth_rule_name}"
...
resource "azurerm_servicebus_topic" "servicebus_topic" {
  ...
  name         = "sbt-wpp-wt-ampm-${var.client_name}-${module.location-lookup.location-lookup["location_short"]}-${var.environment}-${each.value.topic_name}"
  namespace_id = azurerm_servicebus_namespace.servicebus_namespace[each.value.namespace_name].id
...
resource "azurerm_servicebus_topic_authorization_rule" "servicebus_topic_authorization_rule" {
  for_each = { for k, v in local.topic : k => v }
  name     = "sbt-authrule-wpp-wt-ampm-${var.client_name}-${module.location-lookup.location-lookup["location_short"]}-${var.environment}-${each.value.auth_rule_name}"
...
resource "azurerm_servicebus_subscription" "servicebus_subscription" {
  for_each           = { for k, v in local.topic : k => v }
  name               = "sbts-wpp-wt-ampm-${var.client_name}-${module.location-lookup.location-lookup["location_short"]}-${var.environment}-chat-logger"
  topic_id           = azurerm_servicebus_topic.servicebus_topic[each.key].id
